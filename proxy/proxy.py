...
PROXY_TUNNEL_ESTABLISHED_RESPONSE_PKT = CRLF.join(
    [b"HTTP/1.1 200 Connection established", PROXY_AGENT_HEADER, CRLF]
)
...
PROXY_AUTHENTICATION_REQUIRED_RESPONSE_PKT = (
    CRLF.join(
        [
...
class ChunkParser(object):
    ...
    states = namedtuple(
        "ChunkParserStates", ("WAITING_FOR_SIZE", "WAITING_FOR_DATA", "COMPLETE")
    )(1, 2, 3)
...
class HttpParser(object):
    ...
    types = namedtuple("HttpParserTypes", ("REQUEST_PARSER", "RESPONSE_PARSER"))(1, 2)
    ...
    def is_chunked_encoded_response(self):
        return (
            ...
            and b"transfer-encoding" in self.headers
    ...
    def process(self, data):
        ...
        ) and (self.method == b"POST" or self.type == HttpParser.types.RESPONSE_PARSER):
            if not self.body:
                self.body = b""
        ...
        # `TestHttpParser.test_response_parse_without_content_length` for details
            ...
            and self.method == b"POST"
            and (
                b"content-length" not in self.headers
                or (
                    b"content-length" in self.headers
                    and int(self.headers[b"content-length"][1]) == 0
                )
            )
            and self.raw.endswith(CRLF * 2)
        ):
...
class Proxy(threading.Thread):
    ...
    def __init__(
        self, client, auth_code=None, server_recvbuf_size=8192, client_recvbuf_size=8192
    ):
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HttpParser.states.COMPLETE:
            ...
            if self.auth_code:
                if (
                    b"proxy-authorization" not in self.request.headers
                    or self.request.headers[b"proxy-authorization"][1] != self.auth_code
                ):
    ...
    def run(self):
        ...
        try:
            ...
            logger.debug(
                "closing client connection with pending client buffer size %d bytes"
                % self.client.buffer_size()
            )
            self.client.close()
            if self.server:
                logger.debug(
                    "closed client connection with pending server buffer size %d bytes"
                    % self.server.buffer_size()
                )
            self._access_log()
            logger.debug(
                "Closing proxy for connection %r at address %r"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Having difficulty using proxy.py? Report at: %s/issues/new"
        % __homepage__,
    )
